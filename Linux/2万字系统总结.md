...
![图片](2万字系统总结.assets/640-20220302130620313.jpeg)
...
![图片](2万字系统总结.assets/640-20220302130620340.jpeg)
...
![图片](2万字系统总结.assets/640-20220302130620324.jpeg)
...
![图片](2万字系统总结.assets/640-20220302130620338.jpeg)
...
![图片](2万字系统总结.assets/640-20220302130620323.jpeg)
...
![图片](2万字系统总结.assets/640-20220302130620321.jpeg)
...
在 `Linux` 中一个命令的去向可以有3个地方：终端、文件、作为另外一个命令的入参。![图片](2万字系统总结.assets/640-20220302130620343.jpeg)命令一般都是通过键盘输入，然后输出到终端、文件等地方，它的标准用语是 `stdin` 、 `stdout` 以及 `stderr` 。
...
![图片](2万字系统总结.assets/640-20220302130620338-6197580.jpeg)
...
![图片](2万字系统总结.assets/640-20220302130620350.jpeg)
...
![图片](2万字系统总结.assets/640-20220302130620346.jpeg)
...
![图片](2万字系统总结.assets/640-20220302130620368.jpeg)
...
我们来编译安装 `htop` 软件，首先在它的官网下载源码：bintray.com/htop/source…<sup style="line-height: 0;color: #1e6bb8;font-weight: bold;">[1]</sup>
...
```
:r filename # 可以用Tab键来自动补全另外一个文件的路径复制代码
```
...
```
1. ctrl + v 进入块可视模式2. 使用方向键进行选中（上下左右）假设选中5行3. 输入 I 键进行多行同时插入操作4. 插入完成后连续按两下 esc 键，实现多行同时插入相同字符复制代码
```
...
```
set number " 显示行号syntax on " 激活语法高亮set showcmd " 实时看到输入的命令set ignorecase " 搜索时不区分大小写set mouse=a " 激活鼠标，用鼠标选中时相当于进入可视模式复制代码
```
